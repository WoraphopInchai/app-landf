// =========================================================
// AI Matching core (ฝั่ง Cloud Functions) — โมเดลเดียวกับ client lib เดิม
// แต่ปรับเป็น budget/โควตาที่เป็นจริง: คีย์ฟรี Gemini = ~20 ครั้ง/วัน
// => ค่า default ทั้งหมดตั้งแบบประหยัด (conserve) เปลี่ยนที่เดียวได้เมื่อเปิด billing
// =========================================================
...
// โมเดลใหม่: AI ใช้เฉพาะ "extract aiData ครั้งเดียวต่อโพสต์" (1 call/โพสต์, cache ลด)
// การจับคู่เป็นกฎ deterministic ฟรีทั้งหมด (ดู ruleScore) => ระบบแมทได้ทั้งวันไม่ถูกโควตากัด
export const POSTS_PER_RECONCILE = 8; // โพสต์ที่ reconcile ประมวลผลต่อรอบ (กฎฟรี -> ได้)
export const DAILY_GEMINI_CAP = 20; // หยุดเรียก Gemini (extract) เมื่อถึง cap ฟรี
export const USER_EXTRACT_CAP_PER_DAY = 5; // extract ต่อคน/วัน (กันสแปมโพสต์เผาโควตา)
...
// หลักฐานว่าทั้ง 2 โพสต์เป็น "ของชนิดเดียวกัน" (กันจัดหมวดหมู่ผิด + กันแมทข้ามชนิด)
...
// ---------- ตัวให้คะแนนคู่แมทด้วยกฎ (ฟรี ไม่ใช้โควตา Gemini) ----------
// แทน Gemini compare รายคู่ (ประหยัดโควตา + แมทได้ทั้งวัน)
// ผู้ใช้เป็นคนตัดสินสุดท้ายเสมอ: มองว่า "เลขนี้คือโอกาสใกล้เคียง" ไม่ใช่ความจริงที่ AI ฟันธง
...
const ruleScore = (
  ...
  // หมวดต่างชัดเจน + ไม่มีหลักฐานของชนิดเดียวกัน: ปรับทรุดแรง (กันเดาแมทข้ามชนิด)
  if (clearConflict && !ev.hasEvidence) score -= 35;

  // ไม่ให้คะแนนสูงเกินจริง: แมทจริงต้องผ่านหลักฐาน (evidence) เท่านั้น
...
// ---------- Budget รายวัน 2 ชั้น (กันเผาโควตาฟรี 20/วัน) ----------
// ชั้นที่ 1: โปรเจกต์รวม (meta/geminiBudget) — เทียบ quota ฟรีของคีย์/โมเดล
// ชั้นที่ 2: ต่อคน (meta/geminiUsage/{uid}) — กันคนเดียวอัดโพสต์เยอะจนเผา quota รวม
...
export async function readGeminiBudget(
  ...
  try {
    ...
    // อ่านไม่ได้ ให้ถือว่าใช้ได้ (function จะค่อย fail เองตอน 429)
  }
  if (uid) {
    try {
...
export async function recordGeminiCall(
  ...
  try {
    ...
    // ถ้าเขียนไม่ได้ ไม่ควรพังทั้ง flow
  }
  if (uid) {
    try {
      const usnap = await db.doc(`${USER_BUDGET_BASE}/${uid}`).get();
...
// ---------- ตัวแมทหลัก (server) ----------
// Gemini ใช้เฉพาะ "extract aiData" (ครั้งเดียวต่อโพสต์) — จับคู่ทั้งหมดด้วย ruleScore ฟรี
export async function runAiMatchServer(args: {
  ...
  // 1.1 extract aiData (คล้าย client ตั้งใจรู้จัก "ของจริง" ของโพสต์)
  ...
  // 2. ดึงโพสต์ฝั่งตรงข้าม (มี fallback ถ้าไม่มี composite index)
  const fetchOpposite = async (): Promise<
    Array<admin.firestore.QueryDocumentSnapshot<admin.firestore.DocumentData>>
  > => {
    try {
      const snap = await db
        .collection("posts")
  ...
  // หมวดต่างชัดเจน + ไม่มีหลักฐานของชนิดเดียวกัน => block ไม่เสนอเป็นคู่ (ruleScore จะปรับทรุดแรงด้วย)
  ...
  // ทั้งหมดเป็นกฎฟรี => ประเมิน candidate ทุกตัว (ไม่ต้องตัด cap)
  const aiDocs = ranked;
  ...
  for (const r of aiDocs) {
    ...
    // ข้ามคู่ที่ settled / rejected / เพิ่ง judge ซ้ำ
    ...
    // กันแมทข้ามหมวดที่ไม่มีหลักฐาน (ไม่เสียโควตา)
    if (softBlockedIds.has(docSnap.id)) continue;

    // กฎ deterministic ฟรี (ไม่เรียก Gemini): มีหลักฐานชัดเจนถึงขยับเป็นแมทจริง
    const rs = ruleScore(post, t, storedAiData);
    const score = rs.score;
    const reason = rs.reason;

    // สถานะของคู่นี้: กันเสนอคู่ที่โดน judge ไปแล้วซ้ำ (เขียนทั้ง 2 ฝั่ง)
    const nowPair = Date.now();
    judgedAt[docSnap.id] = new Date(nowPair).toISOString();

    // แมทจริง (เขียว) เจ้าของจะกดยืนยันเองเสมอ — และระบบเองต้องมีหลักฐานว่าของชนิดเดียวกัน
    // (หมวดตรง + คำสำคัญ/ข้อความใกล้) ถึงจะเสนอเป็นคู่แมท — กันเดาแมทข้ามชนิด
    ...
    if (gatePassed) {
      ...
      // ต่ำกว่า near: แค่บันทึกว่า judge แล้ว (กันเทียบซ้ำ) — ไม่ต้อง mirror
      toMirror.push({
        docId: docSnap.id,
  ...
  // --- เขียนผลกลับ (auto scan + mirror + judged) ---
  ...
  // ล้าง mirror เก่า (คู่ที่ไม่มีในผลใหม่อีกแล้ว)
  ...
  // เขียน mirror ใหม่ (รวม judged ทั้ง 2 ฝั่ง)
  ...
  // กัน aiJudgedAt โตไม่จำกัด: เก็บเฉพาะ 60 วันล่าสุด
